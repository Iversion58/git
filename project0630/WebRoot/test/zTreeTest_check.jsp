...
<!--
	...
	var setting = {
			...
			data : {
				simpleData : {
					enable : true,
					idKey : "id",//指定结点中哪个属性是结点id
					pIdKey : "parentid",//指定结点中哪个属性是父结点id
					rootPId : 0
				},
				key: {
					url: "xUrl"
	...
	$(document).ready(
			function() {
				//重点学习createSyncTree类的使用
				//第一个参数：div的id即树显示的区域id
				//第二个参数：树区域中ul的id
				//第三个参数：要显示树结点名称的input框
				//第四个参数：非常重要，要存储树结点id，中间以逗号分隔
				//第五个参数：setting设置信息
				//第六个参数：树结点，实际使用此结点数据从服务端获取json
				//第七个参数：允许选择的树的层次
				//第八个参数：事件类型：onCheck（用于多选，onClick用于单击结点，）
				//第九个参数：默认选中的结点，注意：要以逗号分隔，并且只传入子结点
