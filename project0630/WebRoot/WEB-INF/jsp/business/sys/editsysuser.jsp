...
<BODY>
<form id="sysusereditform" name="sysusereditform" action="${baseurl}/sys/user/updatesubmit.action" method="post">
<input type="hidden" name="sysuserCustom.id" value="${id}"/>
						...
						<TBODY>
							...
							<TR>
								...
								<c:forEach items="${userStateList}" var="dictinfo">
								   <input type="radio" name="sysuserCustom.userstate" value="${dictinfo.id}" <c:if test="${sysuserCustom.userstate== dictinfo.id}">checked</c:if>/>${dictinfo.info}
								</c:forEach>
